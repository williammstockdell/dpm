...
// Signal handler
extern "C" void dpm_agent_sighandler(int signum, siginfo_t* siginfo, void*) {
...
int setupSignals() {
    ...
    for (std::size_t i = 0; i < signals.size(); ++i) {
        ...
        if (rc < 0) {
            LOG_ERROR_MSG("Error setting up dpm_agent signal handler: " << strerror(errno));
            exit(EXIT_FAILURE);
        }
    }
...
std::string setlogging(std::string& logdir, const std::string& hostname) {
    if (logdir.empty()) {
        // Use default
        logdir = "/var/log";
    }
...
int main(int argc, const char** argv) {
    ...
    if (rlimit_nofile.rlim_cur < 819200) {
        ...
        if (setrlimit(RLIMIT_NOFILE, &rlimit_nofile) < 0) {
            LOG_WARN_MSG("Could not increase file descriptor rlimits, some programs may be resource constrained. Current rlimits setting is " << rlimit_nofile.rlim_cur);
        }
    }
