...
SslConfiguration::SslConfiguration(Use::Value use, Certificate::Value certificate, Properties::ConstPtr properties_ptr)
    : _use(use), _certificate(certificate), _my_private_key_filename(), _ca_certificates_path(), _ca_certificate_filename(), _use_default_paths(false) {
    const string admin_cert_file_name(properties_ptr->getValue(AdministratorCertificateSectionName, "certificate"));

    const string admin_key_file_name(properties_ptr->getValue(AdministratorCertificateSectionName, "key_file"));

    const string command_cert_file_name(properties_ptr->getValue(CommandCertificateSectionName, "certificate"));

    const string command_key_file_name(properties_ptr->getValue(CommandCertificateSectionName, "key_file"));
    ...
    if ((!_ca_certificates_path) && (!_ca_certificate_filename) && (!_use_default_paths)) {
        throw(runtime_error("the SSl configuration is not valid, no CA certificate is configured"));
    }

    LOG_DEBUG_MSG("Configuration:\n"
                  "\tCertificate file: '"
                  << _my_cert_filename
                  << "'\n"
                     "\tPrivate key file: '"
                  ...
                  << (_ca_certificates_path ? string() + "'" + *_ca_certificates_path + "'" : "not set")
                  << "\n"
                     "\tCA certificate file: "
                  << (_ca_certificate_filename ? string() + "'" + *_ca_certificate_filename + "'" : "not set")
                  << "\n"
                     "\tCA use default paths: "
                  << _use_default_paths);
}
