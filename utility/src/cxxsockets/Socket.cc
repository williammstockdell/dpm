...
void Socket::pBind(const SockAddr& addr) {
    const int reuse = 1;
...
void Socket::getSockName(SockAddr& sa) const {
    FileLocker locker;
    LockFile(locker);
...
void Socket::getPeerName(SockAddr& sa) const {
    FileLocker locker;
    LockFile(locker);
    return internal_getPeerName(sa);
}

bool Socket::Shutdown(const ShutDownSide rw) {
