...
openssl genrsa -out security/ca/key.pem 3072

# Create request for CA certificate
openssl req -new -key security/ca/key.pem -sha256 -nodes -subj '/C=PT/O=DPM/CN=DPM Root CA' -out security/ca/req.pem

# Generate CA certificate
openssl x509 -req -in security/ca/req.pem -sha256 -days 9999 -extensions v3_ca -signkey security/ca/key.pem -out security/ca/cert.pem
...
openssl x509 -req -in security/admin/req.pem -sha256 -days 9999 -extensions usr_cert -CA security/ca/cert.pem -CAkey security/ca/key.pem -CAcreateserial -out security/admin/cert.pem
...
openssl x509 -req -in security/command/req.pem -sha256 -days 9999 -extensions usr_cert -CA security/ca/cert.pem -CAkey security/ca/key.pem -CAcreateserial -out security/command/cert.pem
