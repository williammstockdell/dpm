...
BOOST_AUTO_TEST_CASE(create_from_container) {
    ...
    const std::string qualifier("my qualifier");
    {
        ...
        Container::Timer::Ptr timer = counters_->create()->function(function)->id(id)->otherData(other)->subFunction(sub_function)->qualifier(qualifier);
    }
...
BOOST_AUTO_TEST_CASE(bad_setter_types) {
    // bad ID
    BOOST_CHECK_THROW(counters_->create()->id(Foo()), boost::bad_lexical_cast);

    // bad function
    BOOST_CHECK_THROW(counters_->create()->function(Foo()), boost::bad_lexical_cast);

    // bad other data
    BOOST_CHECK_THROW(counters_->create()->otherData(Foo()), boost::bad_lexical_cast);

    // bad sub function
    BOOST_CHECK_THROW(counters_->create()->subFunction(Foo()), boost::bad_lexical_cast);
}
...
BOOST_AUTO_TEST_CASE(container_not_found) {
    ...
    {
        BOOST_CHECK_THROW(Container::Timer(container, function, id), std::invalid_argument);
    }
}
