...
BOOST_AUTO_TEST_CASE(circular_buffer_size) {
    ...
    BOOST_CHECK_EQUAL(stats.getCapacity(), boost::lexical_cast<size_t>(_properties->getValue("performance", "buffer_capacity")));
}
...
BOOST_AUTO_TEST_CASE(garbage_circular_buffer_size) {
    ...
    // ensure buffer size is sane
    BOOST_CHECK_EQUAL(stats.getCapacity(), PersistentStatisticBuffer::DefaultBufferCapacity);
}
...
BOOST_AUTO_TEST_CASE(buffer_count) {
    ...
    BOOST_CHECK_EQUAL(dataPoints.size(), 1u);
...
BOOST_AUTO_TEST_CASE(buffer_values) {
    ...
    BOOST_FOREACH (const DataPoint& dp, container) {
        ...
        BOOST_CHECK_EQUAL(dp.getFunction(), "test_metric");

        // ensure the id matches
        BOOST_CHECK_EQUAL(dp.getId(), "27");

        // ensure the other data matches
        BOOST_CHECK_EQUAL(dp.getOtherData(), "123");

        // ensure the timer recorded at least three seconds
        BOOST_CHECK(dp.getDuration().total_seconds() >= 3.0);

        // ensure the timer recorded was less than 3.5 seconds
        BOOST_CHECK(dp.getDuration().total_seconds() < 3.5);
    }
}
...
BOOST_AUTO_TEST_CASE(list_values) {
    ...
    BOOST_FOREACH (const DataPoint& dp, container) {
        ...
        // ensure the id matches
        BOOST_CHECK_EQUAL(dp.getId(), "456");

        // ensure the other data matches
        BOOST_CHECK_EQUAL(dp.getOtherData(), "99");

        // ensure the timer recorded at least two seconds
        BOOST_CHECK(dp.getDuration().total_seconds() >= 2.0);

        // ensure the timer recorded was less than 2.5 seconds
        BOOST_CHECK(dp.getDuration().total_seconds() < 2.5);
    }
}
