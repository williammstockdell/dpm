...
BOOST_AUTO_TEST_CASE(file_not_found) { BOOST_CHECK_THROW(Properties::create("/tmp/hopefully_i_do_not_exist"), Properties::FileError); }

BOOST_AUTO_TEST_CASE(malformed_section) {
    _file << "[section" << endl;
    BOOST_CHECK_THROW(Properties::create(_filename), Properties::MalformedSection);
}
...
BOOST_AUTO_TEST_CASE(get_value) {
    ...
    BOOST_CHECK_EQUAL(prop.getValue("section", "key"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key2"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key3"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key4"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key5"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key5"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key6"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key space"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key space2"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key space3"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "key comment"), "value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "value with space"), "value value");
    BOOST_CHECK_EQUAL(prop.getValue("section", "value with space and comment"), "value value");
}
...
BOOST_AUTO_TEST_CASE(get_values) {
    ...
    // compare in order
    BOOST_CHECK(std::equal(section.begin(), section.end(), in_order.begin()));

    // compare out_of_order
    BOOST_CHECK(std::equal(section.begin(), section.end(), out_of_order.begin()) != true);
}
...
BOOST_AUTO_TEST_CASE(duplicate_section) {
    ...
    _file << "[section]" << endl;
    _file << "key=value" << endl;
    BOOST_CHECK_THROW(Properties::create(_filename), Properties::DuplicateSection);
}

BOOST_AUTO_TEST_CASE(missing_section) {
    _file << "key=value" << endl;
    _file << "[section]" << endl;
    BOOST_CHECK_THROW(Properties::create(_filename), Properties::MissingSection);
}
...
BOOST_AUTO_TEST_CASE(comment) {
    ...
    _file << "key=value" << endl;
    BOOST_CHECK_NO_THROW(Properties::create(_filename));
}
...
BOOST_AUTO_TEST_CASE(reload) {
    ...
    BOOST_CHECK_EQUAL(foo.getValue("section", "key"), "value");
    ...
    BOOST_CHECK_THROW(foo.getValue("section_two", "key_two"), std::invalid_argument);

    // reload
    foo.reload();

    // ensure key does not exist after reloading
    BOOST_CHECK_THROW(foo.getValue("section", "key"), std::invalid_argument);

    // ensure key_two exists in section_two
    BOOST_CHECK_EQUAL(foo.getValue("section_two", "key_two"), "value_two");
}

void getValue(Properties::ConstPtr properties, const std::string& section, const std::string& key, const std::string& expected_value, boost::barrier& barrier) {
...
BOOST_AUTO_TEST_CASE(reload_garbage) {
    ...
    // reload should throw
    BOOST_CHECK_THROW(foo.reload(), std::exception);
...
BOOST_AUTO_TEST_CASE(reload_relative_path) {
    Properties foo(_filename);
    BOOST_CHECK_THROW(foo.reload("some/relative/path/to/bg.properties"), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(reload_fully_qualified_path) {
    ...
    Properties foo(_filename);
    BOOST_CHECK_EQUAL(foo.reload(std::string(cwd) + "/" + _filename), true);
