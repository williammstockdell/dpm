...
void Connect(const bgq::utility::PortConfiguration::Pairs& portpairs, CxxSockets::SecureTCPSocketPtr& result) {
    result.reset();
            ...
            BOOST_FOREACH (const CxxSockets::SockAddr& remote, remote_list) {
                ...
                while (!done) {
                    try {
                        ...
                        result = sock;
                        LOG_DEBUG_MSG("connected to " << remote.getHostAddr() << ":" << remote.getServicePort() << " successfully");
                        done = true;
                        ...
                        LOG_WARN_MSG("connecting to " << remote.getHostAddr() << ":" << remote.getServicePort() << " failed: " << e.what());
                        done = true;
                    }
                }
            }
        } catch (const CxxSockets::Error& e) {
            LOG_INFO_MSG("connect attempt failed to " << portpair.first << ":" << portpair.second << " " << e.what());
        }
    }
...
class Server {
  public:
    Server(boost::asio::io_service& io_service) : _io_service(io_service), _ready(false) {
        ...
        _acceptor_ptr.reset(new bgq::utility::Acceptor(_io_service, server_port_configuration));
    }

    void start() { _acceptor_ptr->start(boost::bind(&Server::_acceptHandler, this, _1)); }
  ...
  private:
    ...
    void _acceptHandler(const bgq::utility::Acceptor::AcceptArguments& args) {
...
int main(int argc, char* argv[]) {
    ...
    bgq::utility::initializeLogging(*s_properties_ptr, bgq::utility::LoggingProgramOptions("ibm.test"), "default");
