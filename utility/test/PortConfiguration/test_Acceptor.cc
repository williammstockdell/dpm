...
class GlobalFixture {
  public:
    ...
    GlobalFixture() : properties_ptr(new bgq::utility::Properties()) {
...
void test_stop_accepted(const bgq::utility::Acceptor::AcceptArguments& args, bgq::utility::Acceptor::AcceptArguments* args_out, bgq::utility::Acceptor& acceptor) {
...
BOOST_AUTO_TEST_CASE(test_stop) {
    ...
    bgq::utility::Acceptor acceptor(io_service, server_port_configuration);

    bgq::utility::Acceptor::AcceptArguments args(bgq::utility::Acceptor::Status::ResolveError);

    acceptor.start(boost::bind(&test_stop_accepted, _1, &args, boost::ref(acceptor)));
...
class TestConnect {
  public:
    TestConnect(boost::asio::io_service& io_service) : _io_service(io_service), _got_ok(false) {
        bgq::utility::ServerPortConfiguration server_port_configuration(0);
        ...
        _acceptor_ptr.reset(new bgq::utility::Acceptor(io_service, server_port_configuration));
    }

    void start() { _acceptor_ptr->start(boost::bind(&TestConnect::_handleAccepted, this, _1)); }
  ...
  private:
    ...
    void _handleAccepted(const bgq::utility::Acceptor::AcceptArguments& args) {
        if (args.status == bgq::utility::Acceptor::Status::NowAccepting) {

            cout << "TestConnect: listening on " << args.endpoints[0] << "\n";

            bgq::utility::ClientPortConfiguration client_port_configuration(args.endpoints[0].port());

            client_port_configuration.setProperties(GlobalFixture::get().properties_ptr, "" /* section_name */
            );

            client_port_configuration.notifyComplete();

            _connector_ptr.reset(new bgq::utility::Connector(_io_service, client_port_configuration));

            cout << "TestConnect: connecting...\n";

            _connector_ptr->async_connect(boost::bind(&TestConnect::_handleConnected, this, _1));
        }
    ...
    void _handleConnected(const bgq::utility::Connector::ConnectResult& cr) { cout << "TestConnect::_handleConnected: err=" << cr.error << "\n"; }
};
